...
This feature was introduced with V0.9.15. It adds an experimental way to manage KeySwitch articulations by using the Text Events CC lane. More specifically, when the manager is enabled (which can be done on a per-track basis), if key names are loaded for the MIDI track/channel (File > Note/CC names in the MIDI Editor) and if a name matches a midi note in the item, a text event will automagically be created in the CC lane. 

{:style="text-align: center"}
![](./0.9.15/articulation_manager.gif)

On the screenshot above, KeySwitches are recorded on a dedicated MIDI Channel so that they can be filtered/hidden easily, for more readability, while the Text Events that correspond are still visible.

As long as OSS is running, these events will also be synchronized (if you edit or delete the MIDI notes with your mouse, the events in the Text lane will follow) :

{:style="text-align: center"}
![](./0.9.15/articulation_sync.gif)
...
V0.9.15 (September, 2024, the 25th) :
