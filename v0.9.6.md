...
The current version is available through [ReaPack](https://reapack.com), from the official ReaTeam/Reascripts repository. It also needs the **JS_ReaScriptAPI** api by Julian Sader and the adapted **ReaImGui** library by Christian Fillion to work, which are both available through Reapack too. If you forget to install them, OSS will remind you to do so anyway.
